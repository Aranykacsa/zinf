...
/* Known-good CRC-32 values (Ethernet/ZIP poly 0xEDB88320).
   Reference values computed via Python: binascii.crc32(data) & 0xFFFFFFFF */

static void test_crc_empty(void) {
    TEST_BEGIN("crc32_empty");
    /* crc32(b"") == 0x00000000 */
    uint32_t got = crc32(NULL, 0);
...
static void test_crc_single_byte(void) {
    ...
    uint8_t data = 0x00;
    /* Python: binascii.crc32(b'\x00') & 0xFFFFFFFF == 0xD202EF8D */
    uint32_t got = crc32(&data, 1);
...
static void test_crc_known_string(void) {
    ...
    /* Python: binascii.crc32(b"123456789") & 0xFFFFFFFF == 0xCBF43926 */
    const uint8_t s[] = "123456789";
    uint32_t got = crc32(s, 9);
...
static void test_crc_full_sector_payload(void) {
    ...
    uint32_t first  = crc32(buf, HEADER_SIZE + PAYLOAD_SIZE);
    uint32_t second = crc32(buf, HEADER_SIZE + PAYLOAD_SIZE);
    /* Must be deterministic */
    ASSERT_EQ(first, second);
    /* Must change when data changes */
    buf[0] ^= 0xFFu;
    uint32_t third = crc32(buf, HEADER_SIZE + PAYLOAD_SIZE);
...
static void test_crc_incremental_sensitivity(void) {
    ...
    for (int byte = 0; byte < 16; byte++) {
        for (int bit = 0; bit < 8; bit++) {
            ...
            uint32_t altered = crc32(buf, sizeof(buf));
